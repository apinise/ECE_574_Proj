...
if {![info exists ::env(BASENAME)] } {
  set basename "default"
	...
	set RUN [getenv TRAIL]
}

#reports
report_timing > reports/${basename}_report_timing_${RUN}.rpt
report_power  > reports/${basename}_report_power_${RUN}.rpt
report_area   > reports/${basename}_report_area_${RUN}.rpt
report_qor    > reports/${basename}_report_qor_${RUN}.rpt

set outputnetlist     outputs/${basename}_netlist_${RUN}.v
set outputconstraints outputs/${basename}_constraints_${RUN}.sdc
set outputdelays      outputs/${basename}_delays_${RUN}.sdf
